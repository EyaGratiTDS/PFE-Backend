...
async function generateProjectLogo(projectName, job) {
  try {
    ...
    console.warn(`⚠️ Error generating logo: ${error.message}`);
    ...
    console.log(`🔄 Fallback logo generated: ${fallbackUrl}`);
    return fallbackUrl;
  }
}

/**
 * 🎯 Endpoint 1: Generation with logo (no save)
...
async function generateVCard(req, res) {
  try {
    ...
    console.log("📥 VCard Generation - Data received:", { 
      job, 
      skills, 
      userId,
    ...
    // ✅ Conversion and validation of userId
    ...
    console.log("🔍 UserId validated:", validUserId, "(type:", typeof validUserId, ")");
    ...
    try {
      ...
      console.error("❌ Error retrieving user:", dbError.message);
    ...
    // 🛠️ Normalize skills (string or array)
    ...
    console.log("🤖 Calling AI service to generate VCard...");
    const { project, vcard, blocks } = await generateVCardFull(job, normalizedSkills, userData);
    
    console.log("✅ Data generated by AI:", {
    ...
    // 🖼️ Generate project logo
    console.log("🖼️ Generating project logo...");
    let projectLogo = null;
    try {
      projectLogo = await generateProjectLogo(project.name, job);
      console.log("✅ Project logo generated:", projectLogo);
    } catch (logoError) {
      console.warn("⚠️ Unable to generate logo:", logoError.message);
      // Continue without logo
    }

    // 🔗 Generate unique URL for VCard (not saved yet)
    const vcardUrl = generateUniqueUrl(vcard.name || userData.name || job);
    console.log("🔗 Unique URL generated:", vcardUrl);

    // 📦 Prepare data for frontend (with URL, userId and logo)
    const vcardData = {
      ...vcard,
      url: vcardUrl,
      userId: validUserId,
    ...
    const projectData = {
      ...project,
      logo: projectLogo || project.logo, // ✅ Use generated logo or AI logo
      userId: validUserId,
      status: 'active'
    };

    // 🎉 Final response - VCard generated with logo but NOT saved
    console.log("🎉 Generation completed - Data ready for frontend");
    ...
    return res.status(200).json({
      ...
      savedToDB: false, // 🚫 Indicator that VCard is NOT saved
      logoGenerated: !!projectLogo, // ✅ Indicator if logo was generated
      timestamp: new Date().toISOString()
    });
...
/**
 * 🎯 Endpoint 2: User action management (accept/regenerate)
...
async function notifyVCardAction(req, res) {
  try {
    ...
    // 🎭 Processing according to action
    if (action === 'accept') {
      // ✅ ACCEPTANCE - Save data received from frontend
      console.log("✅ ACCEPT action - Saving data to DB");
      ...
      // 🖼️ Project logo management (upload to Cloudinary if needed)
      ...
      if (projectData.logo && projectData.logo.startsWith('http')) {
        try {
          ...
          console.log("✅ Logo uploaded to Cloudinary:", finalProjectLogo);
        } catch (uploadError) {
      ...
      const newVCard = await VCard.create({ 
        ...
        status: vcardData.status !== undefined ? vcardData.status : 0,  // ✅ Fallback to 0 (false)
      ...
      console.log("✅ VCard created with ID:", newVCard.id);
      ...
      for (let i = 0; i < blocksData.length; i++) {
        ...
        try {
          ...
          console.log(`✅ Block ${i + 1}/${blocksData.length} created: ${block.name} (${block.type_block})`);
          
        } catch (blockError) {
          console.error(`❌ Error creating block ${i + 1}:`, blockError.message);
          // Continue creating other blocks even if one fails
        }
      }

      console.log(`✅ ${createdBlocks.length}/${blocksData.length} blocks created successfully`);
      
      console.log(`✅ VCard accepted and saved successfully`);
      ...
      // 🗑️ REGENERATION - Delete existing VCard if provided
      console.log("🗑️ REGENERATE action - Preparing for new generation");
      ...
      if (vcardId) {
        ...
        if (vcard) {
          ...
          console.log(`🗑️ ${blocksCount} blocks deleted`);
          ...
          // Delete VCard
          await vcard.destroy();
          console.log(`🗑️ VCard ${vcardId} deleted`);
            ...
            if (remainingVCards === 0) {
              ...
              if (project && project.logoPublicId) {
                try {
                  ...
                  console.log(`🗑️ Logo deleted from Cloudinary: ${project.logoPublicId}`);
                } catch (cloudinaryError) {
                  console.warn("⚠️ Cloudinary logo deletion error:", cloudinaryError.message);
                }
              }
              
              await Project.destroy({ where: { id: projectId } });
              console.log(`🗑️ Project ${projectId} deleted (no more associated VCards)`);
            }
          }
...
/**
 * 🧪 Test function to verify VCard generation with logo
...
async function testVCardGeneration(req, res) {
  try {
    ...
    console.log("🧪 VCard generation test with logo:", { job, skills });
    ...
    console.error("❌ Error during generation test:", error);
